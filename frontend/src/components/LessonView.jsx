...
export default function LessonView({ onDifficultyChange }) {
  ...
  /* ── restart lesson when module changes ── */
  useEffect(() => {
    cancelledRef.current = true
    stopAllAudio()
    if (cancelStreamRef.current) cancelStreamRef.current()
    setLessonText(''); lessonTextRef.current = ''
    setChatHistory([])
    setIsInterrupted(false); setIsAnswering(false); setStreamStatus('')
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (e.key === 'Escape') { stopAllAudio(); stt.stopListening(); isListeningRef.current = false }
      if ((e.key === ' ' || e.key === 'Enter') && !typing && isListeningRef.current) {
        e.preventDefault(); stt.stopListening()
      }
    }
  ...
  const startLesson = useCallback(() => {
    ...
    setIsStreaming(true); setLessonText(''); lessonTextRef.current = ''; setStreamStatus('Lesson load ho rahi hai...')
  ...
  }, [sessionId, currentModule, difficultyLevel, tts])
  ...
  const submitQuestion = useCallback(async (question) => {
    ...
    try {
      ...
      // Handle difficulty change from tutor
  ...
  const handleInterruptSubmit = useCallback(() => {
    ...
    if (!q) return
    stt.stopListening(); isListeningRef.current = false
    submitQuestion(q)
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', position:'relative' }}>
      ...
      <div style={{ padding:'14px 24px', borderBottom:'1px solid var(--border)',
        ...
        {/* Difficulty badge — animates on change */}
        <span
      ...
      <div ref={chatEndRef} style={{ flex:1, overflowY:'auto', padding:'24px',
        ...
        {/* Lesson text — clean plain text only */}
        {lessonText && (
        ...
        <div role="dialog" aria-label="Sawaal poochho — mic khula hai"
          ...
          <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:12 }}>
            <button
              ...
              onClick={() => stt.isListening ? stt.stopListening() : stt.startListening(async t => {
              ...
              style={{ width:60, height:60, borderRadius:'50%', flexShrink:0,
                background: stt.isListening ? 'linear-gradient(135deg,var(--accent),var(--accent2))' : 'var(--card)',
            ...
            <button onClick={() => { stopAllAudio(); stt.stopListening(); setIsInterrupted(false);
              cancelledRef.current = false; isListeningRef.current = false }}
