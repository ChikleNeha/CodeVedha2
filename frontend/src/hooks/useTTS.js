...
// Web Speech API TTS — Human-like Hindi + FAST (1.4x speed)
// Optimized for CodeVedha tutor
...
function pickVoice(voices) {
  // ✅ PRIORITY: Neural voices first (human-like)
  return (
    voices.find(v => v.lang === 'hi-IN' && v.name.toLowerCase().includes('neural')) ||  // Neural Hindi
    voices.find(v => v.lang === 'hi-IN' && v.name.includes('Swati')) ||                // Swati Neural
    voices.find(v => v.lang === 'hi-IN') ||                                           // Any Hindi-IN
    voices.find(v => v.lang === 'en-IN') ||                                           // English India fallback
    voices.find(v => v.lang?.startsWith('hi')) ||
    voices.find(v => v.name?.toLowerCase().includes('india')) ||
    null
  )
...
// Split at sentence boundaries, max 200 chars
function splitChunks(text, max = 200) {
  const sentences = text.match(/[^।.!?]+[।.!?]*/g) || [text]
  const chunks = []
  let cur = ''
